...
/*
Open SQLSatOslo2.pbix

TRUNCATE TABLE dbo.PageStructureData
TRUNCATE TABLE [dbo].[PageRecordData]


*/

Declare @DataBaseName varchar(100) = 'PowerBIInternals'
Declare @FileID smallint = 1
Declare @PageId smallint = 403
...
--EXECUTE CheckDB.[dbo].[prc_ReadPageData] @DatabaseName = N'PowerBIInternals', @FileId = 1, @PageId = @PageId

EXECUTE CheckDB.[dbo].[prc_ReadPageDataToTable] @DatabaseName = @DataBaseName, @FileId = 1, @PageId = @PageId

Delete FROM dbo.PageStructureData
...
-- PageRecordData
Delete FROM [PowerBIInternals].[dbo].[PageRecordData]
WHERE DataBaseName = @DataBaseName
AND FileID = @FileID
AND PageNo = @PageId;
